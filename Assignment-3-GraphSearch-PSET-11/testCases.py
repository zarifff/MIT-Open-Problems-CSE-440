...
def load_map(mapFilename):
    ...
    for line in fileObject:
        ...
        g.addNode(srcNode)
        g.addNode(dstNode)
        g.addEdge(WeightedEdge(srcNode, dstNode, weight))
...
def bruteForceSearch(digraph, start, end, path = None):
    """
    ...
    print 'Current brute force path:', path
    
    if Node(start) == Node(end): return path

    for node in digraph.childrenOf(Node(start)):
        if node not in path:
            newPath = bruteForceSearch(digraph, node.getName(), end, path)
            if newPath != None: return newPath

    return None        

digraph= load_map('mit_map.txt')
#print bruteForceSearch(digraph, 2, 9, path = None)
#print bruteForceSearch(digraph, 32, 56, path = None)
print bruteForceSearch(digraph, 1, 32, path = None)
